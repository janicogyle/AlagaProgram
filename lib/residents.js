...
async function updateWithRetry(db, id, resident) {
  ...
  // Beneficiary control numbers are permanent — never overwrite after assignment.
  if (payload.control_number !== undefined) {
    ...
    if (String(existing?.control_number || '').trim()) {
      delete payload.control_number;
    }
  }
...
export async function createOrUpdateResident(resident, options = {}) {
  const { allowContactMerge = true } = options;
  ...
  const sectorValidation = validateSectorPair(resident);
  if (!sectorValidation.ok) {
    throw new Error(sectorValidation.error);
  }

  const residentPayload = {
    ...resident,
    citizenship: LOCKED_CITIZENSHIP,
    primary_sector: sectorValidation.primarySector,
    secondary_sector: sectorValidation.secondarySector || null,
    ...sectorValidation.flags,
  };

  // Validate required fields
  if (!residentPayload.first_name || !residentPayload.last_name) {
  ...
  try {
    ...
    if (residentPayload.id) {
      const { id, ...payload } = residentPayload;
      return await updateWithRetry(db, id, payload);
    }

    if (!residentPayload.contact_number) {
    ...
    const { data: existing, error: lookupError } = await db
      ...
      .eq('contact_number', residentPayload.contact_number)
      .maybeSingle();
    ...
    if (existing) {
      if (!allowContactMerge) {
        throw new Error('This contact number is already registered');
      }
      return await updateWithRetry(db, existing.id, residentPayload);
    }

    return await insertWithRetry(db, residentPayload);
  } catch (error) {
