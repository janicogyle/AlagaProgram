...
import { getRoleSectorAccess, PORTAL_ROLES } from '@/lib/userRoles';
...
async function requireRoles(request, allowedRoles) {
  ...
  let { data: profile, error: profileError } = await supabaseAdmin
    .from('users')
    .select('id, role, status, full_name, email, sector_access')
    .eq('id', authUser.id)
    .single();

  if (profileError && String(profileError.message || '').includes('sector_access')) {
  ...
  profile.sector_access = getRoleSectorAccess(profile.role, profile.sector_access);
...
export async function requireStaffOrAdmin(request) {
  return requireRoles(request, PORTAL_ROLES);
}
