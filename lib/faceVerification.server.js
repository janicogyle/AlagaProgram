...
function getSelfieMinSharpness() {
  return Number(process.env.FACE_API_SELFIE_MIN_SHARPNESS || 28);
}
...
async function analyzeCanvasImageQuality(image, label, options = {}) {
  ...
  return {
    ...
    issue: getImageQualityIssue(metrics, {
      label,
      minSharpness: getSelfieMinSharpness(),
      minBrightness: 58,
      maxBrightness: 220,
      minContrast: 16,
      ...options,
...
async function detectGlassesInSelfie(image, face) {
  ...
  const maxX = Math.min(image.width, Math.ceil(Math.max(...eyePoints.map((point) => point.x)) + box.width * 0.16));
  const minY = Math.max(0, Math.floor(Math.min(...eyePoints.map((point) => point.y)) - box.height * 0.08));
  const maxY = Math.min(image.height, Math.ceil(Math.max(...eyePoints.map((point) => point.y)) + box.height * 0.12));
...
async function verifyWithFaceApi({ idImageUrl, alternateIdImageUrl = '', selfieUrl }) {
  ...
  try {
    ...
    const selfieQuality = await analyzeCanvasImageQuality(selfieImage, 'Selfie');
    ...
    const selfieGlasses = await detectGlassesInSelfie(selfieImage, selfieFaceResult.face);
