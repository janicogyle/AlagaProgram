...
export function isUniSmsConfigured() {
...
export function canSendSms() {
  if (isUniSmsConfigured()) return true;
  return isSmsDevModeEnabled() && process.env.NODE_ENV !== 'production';
}

export function getSmsSetupError() {
  if (isUniSmsConfigured()) return null;
  if (isSmsDevModeEnabled() && process.env.NODE_ENV === 'production') {
    return 'SMS_DEV_MODE cannot be used in production. Set a real UNISMS_API_KEY.';
  }
  return `SMS is not configured. Sign up at ${UNISMS_SIGNUP_URL}, copy your API Secret key, and set UNISMS_API_KEY in .env.local. For local testing without credits, set SMS_DEV_MODE=true.`;
}

function getSmsConfig() {
  const apiKey = String(process.env.UNISMS_API_KEY || '').trim();
  const senderId = process.env.UNISMS_SENDER_ID || '';
  const apiUrl = process.env.UNISMS_API_URL || DEFAULT_API_URL;

  if (!isUniSmsConfigured()) {
    throw new Error(getSmsSetupError() || 'Missing UNISMS_API_KEY.');
  }
...
export async function sendSms({ to, message, referenceType, referenceId, referenceKey }) {
  const contactNumber = normalizeSmsContactNumber(to);
  if (!contactNumber) {
    throw new Error('Invalid recipient contact number.');
  }

  const { apiKey, senderId, apiUrl } = getSmsConfig();
  ...
  try {
    ...
    console.info(`[SMS] UniSMS POST ${url} → ${apiDigits} (${recipient})`);
    ...
    console.error(`[SMS] UniSMS network error for ${apiDigits}:`, err?.message || err);
    await recordSmsLog({
      contactNumber,
      message,
      status: 'failed',
  ...
  const logResult = await recordSmsLog({
    contactNumber,
    message,
    status: 'sent',
    provider: 'unisms',
