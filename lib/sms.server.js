...
const PLACEHOLDER_SENDER_IDS = new Set([
  'your_sender_id',
  'your-sender-id',
  'sender_id',
  'sender-id',
...
function isConfiguredSenderId(value) {
  const senderId = String(value || '').trim();
  if (!senderId) return false;
  if (PLACEHOLDER_SENDER_IDS.has(senderId.toLowerCase())) return false;
  return true;
}

function isOtpSecretConfigured() {
  return !!String(process.env.SMS_OTP_SECRET || process.env.BENEFICIARY_SESSION_SECRET || '').trim();
}
...
export function isUniSmsSenderConfigured({ channel = SMS_CHANNEL_DEFAULT, omitSenderId = false } = {}) {
  const smsChannel = normalizeSmsChannel(channel);
  if (smsChannel === SMS_CHANNEL_LINK || omitSenderId) return true;
  return isConfiguredSenderId(process.env.UNISMS_SENDER_ID);
}
...
export function canSendSms({ channel = SMS_CHANNEL_DEFAULT } = {}) {
  const smsChannel = normalizeSmsChannel(channel);
  if (isUniSmsConfigured({ channel: smsChannel }) && isUniSmsSenderConfigured({ channel: smsChannel })) return true;
...
export function getSmsSetupError({ channel = SMS_CHANNEL_DEFAULT } = {}) {
  ...
  if (isUniSmsConfigured({ channel: smsChannel })) {
    if (isUniSmsSenderConfigured({ channel: smsChannel })) return null;
    return 'SMS is not configured. Set UNISMS_SENDER_ID to an approved UniSMS sender ID, then restart the server.';
  }
...
export function getOtpSetupError() {
  const devMode = isSmsDevModeEnabled() && process.env.NODE_ENV !== 'production';

  if (!isOtpSecretConfigured()) {
    return 'SMS OTP is not configured. Set SMS_OTP_SECRET, then restart the server.';
  }

  if (devMode) return null;

  return getSmsSetupError();
}

function getSmsConfig({ channel = SMS_CHANNEL_DEFAULT, omitSenderId = false } = {}) {
  const smsChannel = normalizeSmsChannel(channel);
  const apiKey =
  ...
  if (!isUniSmsSenderConfigured({ channel: smsChannel, omitSenderId })) {
    throw new Error(getSmsSetupError({ channel: smsChannel }) || 'Missing UNISMS_SENDER_ID.');
  }
...
export async function sendSms({
  ...
  const { apiKey, senderId, apiUrl, timeoutMs, channel: smsChannel } = getSmsConfig({ channel, omitSenderId });
