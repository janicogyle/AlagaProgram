...
export async function getAllAccountRequests() {
  ...
  try {
    const { data, error } = await supabase
      ...
      .select(
        'id, status, first_name, middle_name, last_name, birthday, contact_number, email, house_no, purok, street, barangay, city, is_pwd, is_senior_citizen, is_solo_parent, primary_sector, secondary_sector, representative_name, representative_contact, representative_relationship, representative_valid_id_url, processed_by, processed_at, notes, created_at',
      )
...
export async function approveAccountRequest(requestId, processedBy) {
  ...
  try {
    ...
    const { data: request, error: fetchError } = await supabase
      .from('account_requests')
      .select(
        'id, status, first_name, middle_name, last_name, birthday, contact_number, email, house_no, purok, street, barangay, city, is_pwd, is_senior_citizen, is_solo_parent, primary_sector, secondary_sector, representative_name, representative_contact, representative_relationship, representative_valid_id_url',
      )
      .eq('id', requestId)
      .single();
    ...
    if (existingResident) {
      return { data: null, error: new Error('This contact number is already registered') };
    }
    ...
    const residentData = await createOrUpdateResident({
      ...
      secondary_sector: request.secondary_sector,
      representative_name: request.representative_name,
      representative_contact: request.representative_contact,
      representative_relationship: request.representative_relationship,
      representative_valid_id_url: request.representative_valid_id_url,
      status: 'Active',
    });
...
// Mark account request incomplete
