...
export async function renderBeneficiaryIdCard({
  ...
  void issuedAt;
  const [logo, qrImage, profilePhoto] = await Promise.all([
    loadCanvasImage('/Brand.png'),
    loadCanvasImage(qrUrl),
    profilePhotoUrl ? loadCanvasImage(profilePhotoUrl).catch(() => null) : Promise.resolve(null),
  ]);
  ...
  ctx.fillStyle = '#ffffff';
  roundedRect(ctx, 216, 204, 160, 160, 18);
  ctx.fill();
  ctx.strokeStyle = 'rgba(11, 91, 168, 0.2)';
  ctx.lineWidth = 2;
  roundedRect(ctx, 216, 204, 160, 160, 18);
  ctx.stroke();
  ...
  if (profilePhoto) {
    ctx.save();
    roundedRect(ctx, 226, 214, 140, 140, 14);
    ctx.clip();
    drawImageCover(ctx, profilePhoto, 226, 214, 140, 140);
    ctx.restore();
  } else {
    ctx.fillStyle = '#eaf5ff';
    roundedRect(ctx, 226, 214, 140, 140, 14);
    ctx.fill();
    ctx.fillStyle = '#8fb7dd';
    ctx.beginPath();
    ctx.arc(296, 263, 31, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(296, 327, 55, 34, 0, 0, Math.PI * 2);
    ctx.fill();
  }

  ctx.fillStyle = '#ffffff';
  ctx.fillRect(153, 381, 286, 286);
  ctx.drawImage(qrImage, 163, 391, 266, 266);

  drawFittedText(ctx, String(fullName || 'Beneficiary').toUpperCase(), ID_CARD_WIDTH / 2, 707, 445, {
    color: '#0b5ba8',
    maxSize: 38,
    minSize: 24,
    weight: '800',
  ...
  const sectorText = String(sectorLabel || 'General').trim().toUpperCase() || 'GENERAL';
  const maxPillWidth = ID_CARD_WIDTH - 120;
  let sectorFontSize = 21;
  ctx.font = `700 ${sectorFontSize}px Arial, Helvetica, sans-serif`;

  while (sectorFontSize > 15 && ctx.measureText(sectorText).width + 34 > maxPillWidth) {
    sectorFontSize -= 1;
    ctx.font = `700 ${sectorFontSize}px Arial, Helvetica, sans-serif`;
  }

  const fittedSectorText = truncateText(ctx, sectorText, maxPillWidth - 34);
  const pillWidth = Math.min(maxPillWidth, Math.max(72, ctx.measureText(fittedSectorText).width + 34));
  roundedRect(ctx, (ID_CARD_WIDTH - pillWidth) / 2, 732, pillWidth, 38, 19);
  ctx.fillStyle = '#5b86db';
  ctx.fill();
  ctx.fillStyle = '#ffffff';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText(fittedSectorText, ID_CARD_WIDTH / 2, 751);

  drawCardDetail(ctx, { icon: 'card', label: 'Card No.', value: cardReference || '-', y: 811 });
  drawCardDetail(ctx, { icon: 'calendar', label: 'Expires', value: formatCardDate(expiresAt), y: 862 });
