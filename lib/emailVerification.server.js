...
export function verifyEmailVerificationToken(token, expectedEmail) {
  ...
  if (!token || !normalizedExpectedEmail) {
    return { ok: false, error: 'Gmail verification is required.' };
  }
  ...
  try {
    ...
    return { ok: false, error: 'Gmail verification is not configured.' };
  }

  if (!verified.ok || !verified.payload) {
    return { ok: false, error: 'Gmail verification is invalid. Please verify again.' };
  }
  ...
  if (normalizeVerificationEmail(payload.email) !== normalizedExpectedEmail) {
    return { ok: false, error: 'Verified Gmail does not match the email on this signup.' };
  }

  const now = Math.floor(Date.now() / 1000);
  if (typeof payload.exp === 'number' && payload.exp < now) {
    return { ok: false, error: 'Gmail verification expired. Please verify again.' };
  }
