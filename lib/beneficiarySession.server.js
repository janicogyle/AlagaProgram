...
export const BENEFICIARY_SESSION_MAX_AGE_SECONDS = 60 * 60 * 8;

function getSessionSecret() {
  return process.env.BENEFICIARY_SESSION_SECRET || process.env.QR_CARD_SECRET || '';
}

export function createBeneficiarySessionToken(residentId, { maxAgeSeconds = BENEFICIARY_SESSION_MAX_AGE_SECONDS } = {}) {
...
export function readBeneficiarySession(request) {
  ...
  return readBeneficiarySessionToken(cookie);
}

export function readBeneficiarySessionToken(cookie) {
  if (!cookie) return { ok: false, residentId: null, error: 'Missing session.' };

  let verified;
  try {
    verified = verifyHmacToken(cookie, getSessionSecret());
  } catch {
    return { ok: false, residentId: null, error: 'Invalid session configuration.' };
  }
  ...
  if (typeof payload.iat === 'number' && payload.iat + BENEFICIARY_SESSION_MAX_AGE_SECONDS < now) {
    return { ok: false, residentId: null, error: 'Session expired.' };
  }
