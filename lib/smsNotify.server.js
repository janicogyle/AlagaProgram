import {
  buildAccountResubmissionMessage,
  buildAccountStatusMessage,
  buildAssistanceStatusMessage,
  buildBeneficiaryIdRenewalApprovedMessage,
  buildBeneficiaryIdRenewalIncompleteMessage,
...
export async function sendStatusNotification({
  ...
  referenceKey,
  omitSenderId = false,
  smsChannel = SMS_CHANNEL_DEFAULT,
}) {
  ...
  if (!canSendSms({ channel: smsChannel })) {
    const setupError = getSmsSetupError({ channel: smsChannel }) || 'SMS is not configured.';
...
export async function sendAccountStatusSms({ contactNumber, status, notes, requestId }) {
  const smsStatus = status === 'Rejected' || status === 'Archived' ? 'Incomplete' : status;
  const message = buildAccountStatusMessage({ status: smsStatus, notes });
  const event =
    smsStatus === 'Approved' ? 'account_approved' : smsStatus === 'Incomplete' ? 'account_incomplete' : 'account_status';
...
export async function sendAccountResubmissionSms({ contactNumber, notes, requestId, resubmissionCode }) {
  const message = buildAccountResubmissionMessage({ notes, resubmissionCode });

  return sendStatusNotification({
    event: 'account_resubmission_code',
    contactNumber,
    message,
    referenceType: 'account_resubmission',
    referenceId: requestId,
    referenceKey: `${requestId}:resubmission:${Date.now()}`,
  });
}

export function getAccountResubmissionSmsSetupError() {
  if (canSendSms()) return null;
  return getSmsSetupError() || 'Resubmission SMS is not configured.';
}
...
export async function sendBeneficiaryIdRenewalApprovedSms({ contactNumber, expirationDate, requestId }) {
  return sendStatusNotification({
    event: 'beneficiary_id_renewal_approved',
    contactNumber,
    message: buildBeneficiaryIdRenewalApprovedMessage({ expirationDate }),
    referenceType: 'beneficiary_id_renewal',
    referenceId: requestId,
    referenceKey: `${requestId}:approved:${Date.now()}`,
  });
}

export async function sendBeneficiaryIdRenewalIncompleteSms({ contactNumber, remarks, requestId }) {
  return sendStatusNotification({
    event: 'beneficiary_id_renewal_incomplete',
    contactNumber,
    message: buildBeneficiaryIdRenewalIncompleteMessage({ remarks }),
    referenceType: 'beneficiary_id_renewal',
    referenceId: requestId,
    referenceKey: `${requestId}:incomplete:${Date.now()}`,
  });
}
