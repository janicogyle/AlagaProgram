...
const sanitizeRequirementList = (requirements = []) =>
  requirements
    .map((item) => String(item || '').trim())
    .filter(Boolean)
    .filter((item) => !/^qr\s*code$/i.test(item));

const normalizeRequirements = (value) => {
  if (value == null) return null;
  if (Array.isArray(value)) {
    return sanitizeRequirementList(value);
  }
  if (typeof value === 'string') {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        return sanitizeRequirementList(parsed);
      }
    } catch {
      // fall through
    }
    return sanitizeRequirementList([trimmed]);
  }
  return [];
};
...
export const getRequirementsForType = (type, overrides) => {
  ...
  if (overrides && Object.prototype.hasOwnProperty.call(overrides, type)) {
    return sanitizeRequirementList(overrides[type]);
  }
  return sanitizeRequirementList(assistanceData[type]?.requirements || []);
};

export const getLocalRequirementsMap = () => {
  const requirements = readLocalOverrides().requirements || {};
  return Object.fromEntries(
    Object.entries(requirements).map(([type, list]) => [type, sanitizeRequirementList(list)]),
  );
};
...
export const saveLocalRequirements = (type, requirements) => {
  ...
  const next = {
    ...
    requirements: {
      ...
      [type]: sanitizeRequirementList(requirements),
    },
  };
