...
export async function fileToIdentityImage(file, label = 'ID image') {
  ...
  if (file.size > MAX_FILE_SIZE) {
    throw Object.assign(new Error(`${label} must not exceed 2 MB.`), { code: 'INVALID_FILE', status: 400 });
  }
  ...
  if (sharpness < MIN_SHARPNESS) {
    throw Object.assign(
      new Error(`${label} appears blurry. Retake it in good light and keep the camera steady.`),
      { code: 'BLURRY_IMAGE', status: 422 },
    );
  }
...
async function callOcrSpace(image) {
  ...
  if (!apiKey) {
    throw Object.assign(new Error('Identity OCR is not configured.'), { code: 'OCR_NOT_CONFIGURED', status: 503 });
  }
  ...
  try {
    ...
    const message = error?.name === 'AbortError'
      ? 'Identity OCR timed out. Please try again.'
      : 'Identity OCR is temporarily unavailable. Please try again.';
  ...
  if (!response.ok || !payload || payload.IsErroredOnProcessing || !Array.isArray(payload.ParsedResults)) {
    const message = errorMessage(payload?.ErrorMessage) || 'OCR could not read this ID. Retake a clearer image.';
  ...
  if (parsedText.replace(/\s/g, '').length < 30) {
    throw Object.assign(new Error('OCR found too little readable text. Retake a clearer, closer image.'), {
      code: 'INCOMPLETE_OCR',
      status: 422,
    });
  }
...
export async function verifyIdentityWithOcr({ primaryFile, reverseFile = null, profile = {} }) {
  const primary = await fileToIdentityImage(primaryFile, 'Primary ID image');
  const reverse = reverseFile ? await fileToIdentityImage(reverseFile, 'Reverse ID image') : null;
  const texts = await Promise.all([callOcrSpace(primary), reverse ? callOcrSpace(reverse) : Promise.resolve('')]);
  const parsed = parsePhilippineIdentityText(texts.filter(Boolean).join('\n'));
  ...
  if (parsed.missingFields.length) {
    if (!reverse) {
      return {
        ok: false,
        code: 'SECOND_SIDE_REQUIRED',
        message: 'The ID was recognized, but required details may be on the reverse side.',
    ...
    throw Object.assign(
      new Error(`OCR could not extract the required ${parsed.missingFields.join(', ')}. Retake clearer ID images.`),
...
export function verifyIdentityOcrToken(token, profile = {}) {
  ...
  try {
    ...
    return { ok: false, error: 'Identity OCR is not configured.' };
  }
  if (!verified.ok || verified.payload?.typ !== TOKEN_TYPE) {
    return { ok: false, error: 'Identity OCR verification is invalid. Please verify the ID again.' };
  }
  const payload = verified.payload;
  const now = Math.floor(Date.now() / 1000);
  if (!Number.isFinite(payload.exp) || payload.exp < now) {
    return { ok: false, error: 'Identity OCR verification expired. Please verify the ID again.' };
  }
  ...
  if (payload.profileName !== expectedName || payload.profileBirthDate !== normalizeBirthDate(profile.birthDate)) {
    return { ok: false, error: 'Beneficiary details changed after OCR. Please verify the ID again.' };
  }
...
export async function hashRemoteIdentityImage(url) {
  ...
  if (bytes.length > MAX_FILE_SIZE) throw new Error('Verified ID image exceeds 2 MB.');
